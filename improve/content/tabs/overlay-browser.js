...
(function() {
  ...
  var tcm = {
    handleEvent: function tcm__handleEvent(aEvent) {
      switch (aEvent.type) {
        case "load":
          setTimeout(this.init.bind(this), 500);
          break;
  ...
  window.addEventListener("load", tcm, false);
  window.addEventListener("load", undoclose_obj, false);
  window.addEventListener("unload", undoclose_obj, false);
  document.addEventListener("clearrecentlyclosedtabs", undoclose_obj, false);
})();
